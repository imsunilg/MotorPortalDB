...
"$PSQL" -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d SGInsuranceDB -v ON_ERROR_STOP=1 -f "$SCRIPT_DIR/scripts/01_create_schema.sql"

echo "==> 02_tables/*.sql"
for f in "$SCRIPT_DIR"/scripts/02_tables/*.sql; do
    echo "    $f"
    "$PSQL" -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d SGInsuranceDB -v ON_ERROR_STOP=1 -f "$f"
done

echo "==> 03_constraints_indexes.sql"
"$PSQL" -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d SGInsuranceDB -v ON_ERROR_STOP=1 -f "$SCRIPT_DIR/scripts/03_constraints_indexes.sql"
...
"$PSQL" -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d SGInsuranceDB -v ON_ERROR_STOP=1 -f "$SCRIPT_DIR/scripts/06_seed_data.sql"
